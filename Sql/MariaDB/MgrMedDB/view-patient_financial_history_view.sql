...
DROP TABLE IF EXISTS `patient_financial_history_view`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `patient_financial_history_view` AS SELECT
...
ORDER BY p.LastName, p.FirstName, v.VisitDate DESC 
;
