...
DROP TABLE IF EXISTS `visit_revenue_projections_view`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `visit_revenue_projections_view` AS SELECT
    v.Id AS VisitId,
    v.VisitDate,
    v.PatientId,
    ...
    END AS HistoricalCollectionRate,
