...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `patient_insurance_view` AS SELECT
    ...
    a.Id AS CompanyAddressId,
    a.Maildrop AS CompanyMaildrop,
    a.Street AS CompanyStreet,
    a.Suite AS CompanySuite,
    a.ZipCode AS CompanyZipCode,
    a.City AS CompanyCity,
    a.State AS CompanyState,
    a.Country AS CompanyCountry
