...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `billing_view` AS SELECT
    ...
    (SELECT IFNULL(SUM(pay.Amount), 0)
     FROM payments pay
     JOIN charges c3 ON c3.Id = pay.ChargeId
     WHERE c3.VisitId = v.Id) AS TotalPayments,
