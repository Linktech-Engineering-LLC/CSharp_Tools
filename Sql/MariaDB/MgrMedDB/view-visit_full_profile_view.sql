...
DROP TABLE IF EXISTS `visit_full_profile_view`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `visit_full_profile_view` AS SELECT
    ...
    (SELECT MIN(c.ChargeDate) FROM charges c WHERE c.VisitId = v.Id) AS FirstChargeDate,
    (SELECT MAX(c.ChargeDate) FROM charges c WHERE c.VisitId = v.Id) AS LastChargeDate,

    
    (
        (SELECT IFNULL(SUM(c.Amount), 0)
         FROM charges c
         WHERE c.VisitId = v.Id)
        -
        (SELECT IFNULL(SUM(pay.Amount), 0)
         FROM payments pay
         JOIN charges c2 ON c2.Id = pay.ChargeId
...
LEFT JOIN staff s ON s.Id = v.DoctorId
LEFT JOIN facilities f ON f.Id = v.FacilityId
