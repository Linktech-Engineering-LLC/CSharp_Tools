...
DROP TABLE IF EXISTS `visit_claim_lifecycle_view`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `visit_claim_lifecycle_view` AS SELECT
    ...
    CASE
        ...
        THEN 'Closed'
        WHEN (SELECT COUNT(*) FROM payments pay
              JOIN charges c2 ON c2.Id = pay.ChargeId
              WHERE c2.VisitId = v.Id) > 0
        THEN 'In Payment'
        WHEN (SELECT COUNT(*) FROM charges c WHERE c.VisitId = v.Id) > 0
        THEN 'Submitted'
        ELSE 'Unbilled'
    END AS ClaimLifecycleStage
