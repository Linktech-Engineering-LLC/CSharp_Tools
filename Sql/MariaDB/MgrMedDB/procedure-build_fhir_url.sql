...
DROP PROCEDURE IF EXISTS `build_fhir_url`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `build_fhir_url`(
...
BEGIN
    ...
    CALL replace_placeholders(v_path, p_patient_id, p_generic_id, p_prescription_id, v_final);

    
    SET p_full_url = CONCAT(v_base, v_final);
END//
DELIMITER ;
