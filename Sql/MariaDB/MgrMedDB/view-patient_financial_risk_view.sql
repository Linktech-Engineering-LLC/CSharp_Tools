...
DROP TABLE IF EXISTS `patient_financial_risk_view`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `patient_financial_risk_view` AS SELECT
    p.Id AS PatientId,
    p.FirstName,
    p.LastName,
    ...
    CASE
        ...
        WHEN (
            ...
            (SELECT IFNULL(SUM(adj.Amount), 0)
             FROM adjustments adj JOIN visits v2 ON v2.Id = adj.VisitId
             WHERE v2.PatientId = p.Id)
        ) > 250 THEN 'Medium Risk'
        ELSE 'Low Risk'
    END AS FinancialRiskCategory

FROM patients p

ORDER BY PatientBalance DESC, p.LastName, p.FirstName 
;
